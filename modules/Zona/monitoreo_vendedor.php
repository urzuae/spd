...
$sql  = "SELECT gid FROM users WHERE uid='$uid'";
$result = $db->sql_query($sql) or die("Error");
list($gid) = $db->sql_fetchrow($result);
$sql  = "SELECT name FROM groups WHERE gid='$gid'";
$result = $db->sql_query($sql) or die("Error");
list($nombre_concesionaria) = $db->sql_fetchrow($result);
...
<div id=\"bloque_$campana_id\" style=\"display:block;\">";
    ...
    $tabla_campanas .= "<table class=\"width100\">"
                    ."<thead>"
            ."<tr>"
                    ."<td style=\"width:180px;\"><az href=\"index.php?_module=$_module&_op=$_op&uid=$uid&campana_id=$campana_id&gid=$gidorderby=origen_id\" style=\"color:white;\">Campaña</az></td>"
            ."<td style=\"width:360px;\"><az href=\"index.php?_module=$_module&_op=$_op&uid=$uid&campana_id=$campana_id&gid=$gidorderby=nombre\" style=\"color:white;\">Nombre</az></td>"
//            ."<td><az href=\"index.php?_module=$_module&_op=$_op&uid=$uid&campana_id=$campana_id&gid=$gidorderby=tel\" style=\"color:white;\">Teléfono</az></td>"
            ."<td style=\"width:50px;\"><az href=\"index.php?_module=$_module&_op=$_op&uid=$uid&campana_id=$campana_id&gid=$gidorderby=ultimo_contacto\" style=\"color:white;\">Espera</az></td>"
            ."<td style=\"width:180px;\"><az href=\"index.php?_module=$_module&_op=$_op&uid=$uid&campana_id=$campana_id&gid=$gidorderby=primer_contacto\" style=\"color:white;\">Primer contacto</az></td>"
            ."<td style=\"width:180px;\"><az href=\"index.php?_module=$_module&_op=$_op&uid=$uid&campana_id=$campana_id&gid=$gidorderby=ultimo_contacto\" style=\"color:white;\">Último contacto</az></td>"
            ."<td style=\"width:180px;\"><az href=\"index.php?_module=$_module&_op=$_op&uid=$uid&campana_id=$campana_id&gid=$gidorderby=fecha_cita\" style=\"color:white;\">Compromiso</az></td>"
            ."<td style=\"width:180px;\"><az href=\"index.php?_module=$_module&_op=$_op&uid=$uid&campana_id=$campana_id&gid=$gidorderby=fecha_cita\" style=\"color:white;\">Retraso</az></td>"
//             ."<td style=\"width:32px;\">Sel.</td>"
                    ."</tr></thead>"
                    ."<tbody>";
	foreach ($ordered_contacto_ids AS $contacto_id)
	{
      ...
      $tabla_campanas .= "<tr class=\"row".(($c++%2)+1)."\">"
						 ."<td>$origen</td>"
                         ."<td><az href=\"index.php?_module=Campanas&_op=llamada_ro&llamada_id={$llamada_ids[$contacto_id]}&contacto_id=$contacto_id&campana_id={$campana_ids[$contacto_id]}\" target=\"llamada\">
