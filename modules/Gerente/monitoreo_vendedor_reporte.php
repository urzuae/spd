...
$filename = "reporte_vendedor_$uid.csv";
$fp = fopen("$filename", 'w');
...
while (list($campana_id, $name) = ($db->sql_fetchrow($result)))
...
, UNIX_TIMESTAMP(c.fecha_cita),
          d.tel_casa, d.tel_oficina, d.tel_movil, d.tel_otro, d.email, c.`timestamp`, c.status_id
...
<div id=\"bloque_$campana_id\" style=\"display:block;\">";
	...
	$counter = 0;
    while (list($llamada_id, $campana_id, $origen_id, $contacto_id, $nombre, $apellido_paterno, $apellido_materno, $fecha_cita, $fecha_cita_timestamp, $tel_casa, $tel_oficina, $tel_movil, $tel_otro, $email, $llamda_timestamp, $status_id) = $db->sql_fetchrow($r2))
    {
  ...
  fputcsv($fp, array("Campaña","Nombre","Teléfono","e-mail","Espera","Primer contacto","Último contacto","Compromiso","Retraso"));
   ...
   $email = $emails[$contacto_id]; 
    ...
    fputcsv($fp, array("$origen","$nombre $apellido_paterno $apellido_materno","$tel","$email","$espera","$primer_contacto","$ultimo_contacto","$fecha_cita","$retraso")); 
