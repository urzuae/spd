...
$sql = "SELECT queja_id, fecha, uid FROM crm_quejas WHERE status_id=2"; //np=0
...
while (list($queja_id, $fecha, $uid) = htmlize($db->sql_fetchrow($result)))
{
  ...
  $tabla .= "<tr class=\"row".($rowclass++%2+1)."\"><td>$queja_id</td><td>$fecha</td><td>$gname</td></tr>\n";
  $tabla2 .= "<tr><td>$queja_id</td><td>$fecha</td><td>$gname</td></tr>\n";
}
$tabla .= "</tbody></table>";
$tabla2 .= "</tbody></table>";
$body = "Por medio del presente le hago llegar una relación de los folios que no proceden.<br><br>";
