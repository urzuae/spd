...
// PUBLIC METHODS	
    ...
    function StrokeCSIM($aScriptName='auto',$aCSIMName='',$aBorder=0, $return = false) {
	if( $aCSIMName=='' ) {
	...
	if( empty($_GET[_CSIM_DISPLAY]) ) {
        ...
        /*if($return)
            return $this->GetHTMLImageMap($aCSIMName);
        else
            echo $this->GetHTMLImageMap($aCSIMName);*/

        if($return){
            return $this->GetHTMLImageMap($aCSIMName)."<img src='".htmlentities($aScriptName.$urlarg)."' ISMAP USEMAP='#".$aCSIMName.'\' border='.$aBorder.'  width='.$this->img->width.' height='.$this->img->height." alt=\"\" />\n";
        }
        else{
            echo $this->GetHTMLImageMap($aCSIMName);
